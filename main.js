...
function startWindow(){
  if(!fs.readFileSync(__dirname + "/token.json").length){ // if never logged in
...
ipcMain.on('clear-all-data', function (e){
  fs.writeFileSync(__dirname + '/token.json', "");
  var notebooks = JSON.parse(fs.readFileSync(__dirname + '/notebooks.json'));
  notebooks.today_progress = "";
  notebooks.misc_progress = "";
  fs.writeFileSync(__dirname + '/notebooks.json', JSON.stringify(notebooks));
