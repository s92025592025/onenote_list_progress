...
(function (){
	...
	window.onload = function (){
		...
		if(!JSON.parse(fs.readFileSync(__dirname + '/notebooks.json')).today_progress){
	...
	function showToday(){
		...
		onenoteRequest('sections/' 
						+ JSON.parse(fs.readFileSync(__dirname + '/notebooks.json')).today_progress
						+ "/pages"
		...
		timer = setInterval(function (){
				...
				for(var i = 0; i < JSON.parse(fs.readFileSync(__dirname +'/notebooks.json')).misc_progress.length; i++){
					onenoteRequest('sections/' 
									+ JSON.parse(fs.readFileSync(__dirname + '/notebooks.json')).misc_progress[i] 
			...
			}, JSON.parse(fs.readFileSync(__dirname + '/notebooks.json')).refresh_time * 60 * 1000);
